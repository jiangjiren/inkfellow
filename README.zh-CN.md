...
- 在桌面 Markdown 编辑器中可直接粘贴 PNG、JPEG、GIF 或 WebP 图片；20 MB 以内的图片会以防重名文件名保存到笔记同目录，并自动插入相对 Markdown 链接。
...
桌面应用第一次启动时会让你选择 vault，并把选择保存在操作系统的应用配置目录中。若目录还不是 Git 仓库，会自动初始化；同时会在随机 loopback 端口启动打包的 AI 服务。切换 vault 或删除笔记前，应用会等待尚未完成的文本与粘贴图片保存任务，避免数据丢失。

...
- Paste PNG, JPEG, GIF, or WebP images directly into a desktop Markdown note. Images up to 20 MB are saved beside the note with collision-safe names and inserted as relative Markdown links.
...
The desktop app selects a vault on first launch and stores the choice in the OS application config directory. It initializes a local Git repository when necessary, starts the packaged AI service on a free loopback port, and keeps pending note/image saves safe when switching vaults or deleting a note.
